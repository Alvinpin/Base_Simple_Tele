...
module.exports = itsvin = async (itsvin, bot) => {
    //console.log(itsvin)
    try {
        const body = itsvin.message.text || itsvin.message.caption || ''
        const budy = (typeof itsvin.message.text == 'string' ? itsvin.message.text : '')
        const {
            isUrl
        } = simple
        ...
        const user = simple.getUserName(itsvin.message.from)
        const pushname = user.full_name;
        const user_id = itsvin.message.from.id + " "
        const username = itsvin.message.from.username ? itsvin.message.from.username : "alvin_ofc";
        const isCreator = OWNER[0].replace("https://t.me/", '') == itsvin.update.message.from.username
        const from = itsvin.message.chat.id

        const isGroup = itsvin.chat.type.includes('group')
        const groupName = isGroup ? itsvin.chat.title : ''

        const isImage = itsvin.message.hasOwnProperty('photo')
        const isVideo = itsvin.message.hasOwnProperty('video')
        const isAudio = itsvin.message.hasOwnProperty('audio')
        const isSticker = itsvin.message.hasOwnProperty('sticker')
        const isContact = itsvin.message.hasOwnProperty('contact')
        const isLocation = itsvin.message.hasOwnProperty('location')
        const isDocument = itsvin.message.hasOwnProperty('document')
        const isAnimation = itsvin.message.hasOwnProperty('animation')
        const isMedia = isImage || isVideo || isAudio || isSticker || isContact || isLocation || isDocument || isAnimation
        const quotedMessage = itsvin.message.reply_to_message || {}
        ...
        const isQuoted = itsvin.message.hasOwnProperty('reply_to_message')
        ...
        switch (command) {
            ...
            case 'script':
            case 'scrip': {
                itsvin.reply("Source code this bot", {
                    reply_markup: {
                        inline_keyboard: [
                            [{
                                text: 'Shoop',
                                url: "https://shoplinks.to/AlvinStore"
                            }, {
                                text: 'YouTube',
                                url: "https://www.youtube.com/@callmevinz"
                            }]
                        ]
                    }
                })
            ...
            case 'help': {
                ...
                lang.menu(itsvin, THUMBNAIL, pushname, OWNER_NAME, OWNER, prefix, hitall, latensii, os, simple, week, date, dateIslamic, username, isCreator, itsvin.message.from.id.toString())
            }
            break
            //anime
            case "akame":
...
ダ ID : ${search.all[0].videoId}
ダ Title : ${search.all[0].title}
ダ Views : ${search.all[0].views}
ダ Duration : ${search.all[0].timestamp}
ダ Channel : ${search.all[0].author.name}
ダ Upload : ${search.all[0].ago}
ダ URL Video : ${search.videos[0].url}
ダ Description : ${search.videos[0].description}

_Please wait, the audio file is being sent..._`
let todd = await getBuffer(search.all[0].image)
itsvin.sendMessage(m.chat, {image: todd, caption: caption}, {quoted})
...
let mp3file = `./${m.chat}.mp3`
  ...
  .on('finish', async () => {
itsvin.sendMessage(m.chat, {audio: fs.readFileSync(mp3file), mimetype:'audio/mpeg' }, {quoted})
   })
}
break
            case 'yts':
            case 'ytsearch': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} judul lagu/video\n\nContoh penggunaan:\n${prefix+command} bot WhatsApp Alvin`)
                reply(lang.wait)
...
📝 Description : ${dapet[v].description}\n\n---------------------------\n\n`
            ...
            case "mf":
            case "mediafire": {
                if (!args[0]) return reply(`Kirim perintah:\n${prefix+command} link mediafire\n\nContoh penggunaan:\n${prefix+command} https://www.mediafire.com/file/eb14v8x4oz7ok3h/itsvinbot-Mdv17.5-withModule.zip/file`)
                ...
                console.log(size)
                if (size.replace('MB', '') >= 500000 || size.replace('GB', '') >= 1) { //size edit sendiri jika mau download yang lebih media yang lebih besar
                    ...
                    key += `Size: ${size}\n`
                    key += `Link: ${link}\n\n`
                    key += `Sedang Mengompres Silahkan Tunggu.`
                    await reply(key)
                    if (nama.includes(".zip")) {
                        itsvin.replyWithDocument({
                            url: link,
                            filename: nama
                        })
            ...
            case "tiktoknowm":
            case "tiktok": {
                if (!args[0]) return reply(`Kirim perintah:\n${prefix+command} link tiktok video\n\nContoh penggunaan:\n${prefix+command} https://www.tiktok.com/@alvin.official/video/7210229439744003355?_r=1&u_code=e44201c8bfkd30®ion=ID&mid=7202111782981913370&preview_pb=0&language=id&_d=e0cah74j08m7c7&share_item_id=7210229439744003355&source=h5_t×tamp=1679216331&user_id=7148061777321133083&sec_user_id=MS4wLjABAAAA50SieLfP2YD-R-gqSE3svcPxaPqr_53pA6RKyJUkQo_AreOGrLDiVRnajBVglVIk&utm_source=copy&utm_campaign=client_share&utm_medium=android&share_iid=7160625938232592154&share_link_id=6b2fea89-b038-4919-8d58-38b4efba5b9c&share_app_id=1180&ugbiz_name=Main&ug_btm=b8727%2Cb2878`)
                if (!isUrl(args[0]) && !args[0].includes("tiktok.com")) return reply(`Kirim perintah:\n${prefix+command} link tiktok video\n\nContoh penggunaan:\n${prefix+command} https://www.tiktok.com/@alvin.official/video/7210229439744003355?_r=1&u_code=e44201c8bfkd30®ion=ID&mid=7202111782981913370&preview_pb=0&language=id&_d=e0cah74j08m7c7&share_item_id=7210229439744003355&source=h5_t×tamp=1679216331&user_id=7148061777321133083&sec_user_id=MS4wLjABAAAA50SieLfP2YD-R-gqSE3svcPxaPqr_53pA6RKyJUkQo_AreOGrLDiVRnajBVglVIk&utm_source=copy&utm_campaign=client_share&utm_medium=android&share_iid=7160625938232592154&share_link_id=6b2fea89-b038-4919-8d58-38b4efba5b9c&share_app_id=1180&ugbiz_name=Main&ug_btm=b8727%2Cb2878`)
                reply(lang.wait)
                let res = await fetch(global.api('skizo', '/api/tiktok', {
                    url: args[0]
                }, 'apikey'))
                if (!res.ok) throw await res.text()
                var result = await res.json()     
            ...
            /*   case "apple":
               ...
               case "google": {
                   ...
                   let savestik = await itsvin.sendImageAsSticker(m.chat, img, m, {
                       packname: packname,
                       author: author
                   })
            ...
            break
            case 'colorfulpubg': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} teks|color\n\nContoh penggunaan:\n${prefix+command} Alvin|green-yellow`)
                if (!text.includes('|')) return reply(`Kirim perintah:\n${prefix+command} teks|color\n\nContoh penggunaan:\n${prefix+command} Alvin|green-yellow`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'avatar3q360': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} teks|avatar\n\nContoh penggunaan:\n${prefix+command} Alvin|truong_phi`)
                if (!text.includes('|')) return reply(`Kirim perintah:\n${prefix+command} teks|avatar\n\nContoh penggunaan:\n${prefix+command} Alvin|truong_phi`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'overwatchcover': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} teks|hero\n\nContoh penggunaan:\n${prefix+command} Alvin|zsymmetra`)
                if (!text.includes('|')) return reply(`Kirim perintah:\n${prefix+command} teks|hero\n\nContoh penggunaan:\n${prefix+command} Alvin|zsymmetra`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'bannerofaov2': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|banner\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|ishar`)
                if (!text.includes('|')) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|banner\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|ishar`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'gunlogogaming': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|background\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|m14ebr`)
                if (!text.includes('|')) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|background\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|m14ebr`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'banneroffreefire': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|background\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|misha`)
                if (!text.includes('|')) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|background\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|misha`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'bannerofoverwatch': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|background\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|reinhardt`)
                if (!text.includes('|')) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|background\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|reinhardt`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            break
            case 'mascotstyle': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|thumb\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|kitsune`)
                if (!text.includes('|')) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|thumb\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|kitsune`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            case 'avataroverwatch': {
                if (!text) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|thumb\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|zenyatta`)
                if (!text.includes('|')) return reply(`Kirim perintah:\n${prefix+command} teks1|teks2|thumb\n\nContoh penggunaan:\n${prefix+command} alvin|ofc|zenyatta`)
                var mon = args.join(' ')
                var m1 = mon.split("|")[0]
                var m2 = mon.split("|")[1]
            ...
            /*  case "patrick":
              ...
              case "tyni": {
                  ...
                  let res = await fetch(ini_url)
                  if (!res.ok) throw await res.text()
                  itsvin.replyWithSticker({url: ini_url})
              }
              break
              //canvas
              case 'ttp':
              case 'attp': {
                  ...
                  if (!res.ok) throw await res.text()
                  let img = await res.buffer()
                  let encmedia = await itsvin.sendMediaAsSticker(m.chat, img, m, {
...
Contoh:
${prefix}nuliskiri Subscribe Ya YT Alvin`)
                break
            case 'foliokiri':
            case 'foliokanan':
            case 'nuliskanan':
